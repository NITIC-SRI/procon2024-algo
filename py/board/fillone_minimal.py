...
def _get_2pow_formal(i: int) -> int:
    if i == 0:
        return 0
    else:
        return 1 + 3 * (i - 1)
...
def _compress_left(
    h: int, w: int, actions: list, consecutive: int, i: int
) -> tuple[bool, list]:
    action = actions[i - 1]
    if action[3] == "left":
        if action[0] == 0 and action[1] == 0:
            ...
            for idx, j in enumerate(reversed(b[2:])):
                if j == "1":
                    tmp_actions.append(
                        (action[0], 1 - 2**idx, _get_2pow_formal(idx), "left")
                    )
...
def _compress_actions(h: int, w: int, actions: list, func: Callable) -> list:
    compresed_actions = []
    consecutive = 1
...
def compress_actions(h: int, w: int, actions: list) -> list:
